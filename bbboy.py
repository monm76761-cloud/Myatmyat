...
def is_admin(chat_id):
    """ADMIN_ID နှင့် ကိုက်ညီသော Admin ကို Key နှင့် expiration မစစ်ဘဲ ခွင့်ပြုပါ။"""
...
async def start(message):
    ...
    if is_admin(message.chat.id):
        ...
        await bot.reply_to(message, "✅ Admin အနေဖြင့် Key မလိုဘဲ အမြဲအသုံးပြုနိုင်ပါသည်။ /input ဖြင့် Session URL ထည့်ပါ။")
    else:
        await bot.reply_to(message, "Bot စတင်ပါပြီ။ အသုံးပြုရန် Admin ထံမှ ရရှိထားသော Key ဖြင့် /key <key> ကို အသုံးပြုပါ။")
...
async def handle_key(message):
    ...
    supplied_key = args[1].strip() if len(args) > 1 else str(message.chat.id).strip()
    ...
    if matched_key is not None and check_key_expiration(auth_list[matched_key]):
        ...
        await bot.reply_to(message, "⚠️ သင့် Key ကို မှတ်ပုံတင်ထားခြင်း မရှိသေးပါ သို့မဟုတ် သက်တမ်းကုန်ဆုံးနေပါသည်။ Admin ထံ ဆက်သွယ်ပါ။")
