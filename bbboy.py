...
async def handle_key(message):
    ...
    if matched_key is not None:
        if check_key_expiration(auth_list[matched_key]):
            ...
            user_data.setdefault(message.chat.id, {})
            save_state()
            await bot.reply_to(message, "✅ Key မှန်ကန်ပါသည်။\n/setup ဖြင့် Session URL ထည့်ပါ။")
        else:
...
async def handle_setup(message):
    ...
    if len(args) < 2:
        ...
        return
    url = args[1]
    if not is_admin(message.chat.id) and not approve.get(message.chat.id, False):
...
async def main():
    ...
    _connector = aiohttp.TCPConnector(limit=3000, ttl_dns_cache=600, ssl=True, enable_cleanup_closed=True)
